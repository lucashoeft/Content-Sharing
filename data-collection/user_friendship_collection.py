...
user_friendship_list = pd.read_csv('../data/intermediate/user_friendships/user_friendships.csv', sep=";", na_values="")
		...
		if index+1 < len(user_list):
			...
			if target_frienship.empty:
				try:
					...
					user_friendship_list.to_csv('../data/intermediate/user_friendships/user_friendships.csv', index=False, decimal=',', sep=";", float_format='%.f')
				except:
					print("ERROR")

	user_friendship_list.to_csv('../data/intermediate/user_friendships/user_friendships_' + str(source_screen_name) + '.csv' , index=False, decimal=',', sep=";", float_format='%.f')